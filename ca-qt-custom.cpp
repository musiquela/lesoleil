...
// After audio init to get SR
    ...
    // Hardcode window size for Soleil's 3-section UI (Root + Harmony 1 + Harmony 2)
    QMainWindow* mainWindow = interface->findChild<QMainWindow*>();
    if (mainWindow) {
        // Set fixed window size that fits all content
        mainWindow->resize(520, 850);
        mainWindow->setMinimumSize(520, 850);
    }
