/* ------------------------------------------------------------
name: "SGT_HarmonyGenerator_v1.8"
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "SGT_HarmonyGenerator_v1.8.dsp");
		...
		m->declare("name", "SGT_HarmonyGenerator_v1.8");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("SGT_HarmonyGenerator_v1.8");
		...
		ui_interface->addHorizontalSlider("Harmony Control/Manual Semitones", &fHslider1, FAUSTFLOAT(7.0f), FAUSTFLOAT(-24.0f), FAUSTFLOAT(24.0f), FAUSTFLOAT(1.0f));
		ui_interface->declare(&fHslider2, "1", "");
		ui_interface->declare(&fHslider2, "style", "knob");
		ui_interface->addHorizontalSlider("Harmony Control/Wet/Dry Mix", &fHslider2, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		int iSlow1 = static_cast<int>(static_cast<float>(fCheckbox0));
		...
		int iSlow6 = static_cast<int>(static_cast<float>(fCheckbox1));
		float fSlow7 = std::max<float>(1.1920929e-07f, std::fabs(fSlow0));
		float fSlow8 = fConst3 * fSlow7;
		float fSlow9 = 1.0f - fConst0 / fSlow7;
		float fSlow10 = static_cast<float>(static_cast<int>(static_cast<float>(fHslider3)));
		float fSlow11 = 1.0f / static_cast<float>(static_cast<int>(static_cast<float>(fHslider4)));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			fRec0[0] = fSlow2 + fConst2 * fRec0[1];
			float fTemp0 = std::pow(2.0f, 0.083333336f * ((iSlow1) ? fSlow3 : fRec0[0]));
			fHbargraph0 = static_cast<FAUSTFLOAT>(fSlow0 * fTemp0);
			output0[i0] = static_cast<FAUSTFLOAT>(fHbargraph0);
			float fTemp1 = fSlow8 + fRec1[1] + -1.0f;
			int iTemp2 = fTemp1 < 0.0f;
			float fTemp3 = fSlow8 + fRec1[1];
			fRec1[0] = ((iTemp2) ? fTemp3 : fTemp1);
			float fRec2 = ((iTemp2) ? fTemp3 : fSlow8 + fRec1[1] + fSlow9 * fTemp1);
			float fTemp4 = 0.5f * (2.0f * fRec2 + -1.0f);
			float fTemp5 = ((iSlow6) ? fTemp4 : ((iSlow6) ? fTemp4 : static_cast<float>(input0[i0])));
			fVec0[IOTA0 & 131071] = fTemp5;
			fRec3[0] = std::fmod(fSlow10 + fRec3[1] + (1.0f - fTemp0), fSlow10);
			int iTemp6 = static_cast<int>(fRec3[0]);
			float fTemp7 = std::floor(fRec3[0]);
			float fTemp8 = 1.0f - fRec3[0];
			float fTemp9 = std::min<float>(fSlow11 * fRec3[0], 1.0f);
			float fTemp10 = fSlow10 + fRec3[0];
			int iTemp11 = static_cast<int>(fTemp10);
			float fTemp12 = std::floor(fTemp10);
			float fTemp13 = fSlow5 * fTemp5 + fSlow4 * ((fVec0[(IOTA0 - std::min<int>(65537, std::max<int>(0, iTemp6))) & 131071] * (fTemp7 + fTemp8) + (fRec3[0] - fTemp7) * fVec0[(IOTA0 - std::min<int>(65537, std::max<int>(0, iTemp6 + 1))) & 131071]) * fTemp9 + (fVec0[(IOTA0 - std::min<int>(65537, std::max<int>(0, iTemp11))) & 131071] * (fTemp12 + fTemp8 - fSlow10) + (fSlow10 + (fRec3[0] - fTemp12)) * fVec0[(IOTA0 - std::min<int>(65537, std::max<int>(0, iTemp11 + 1))) & 131071]) * (1.0f - fTemp9));
