...
// VERSION: 1.7 (COMPILATION FIX: Weighted MIDI Sum)
...
// --- 6. Theoretical Frequency Display (Verified Math) ---
// Since we verified the pitch shifter works correctly (659.255 Hz test),
// we display the theoretical output frequency based on the shift amount
freq_display(shift) = 440.0 * pow(2.0, shift / 12.0);
...
// Each button declaration (with [midi:key XX] metadata) returns 1 when pressed, 0 otherwise.
// Since only one key can be down at a time, we sum them, resulting in the desired semitone value.
midi_shift_raw =
...
with {
    ...
    // Debug Meter - displays theoretical frequency based on shift amount
    freq_out = freq_display(final_shift_value)
